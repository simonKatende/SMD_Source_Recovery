...
public class ProcessProgressAr : XtraForm
{
	...
	private ArabicNumeralHelper conv = new ArabicNumeralHelper();
	...
	public ProcessProgressAr(string BOT, string MOT, string EOT, bool PROCESSASPER, DateTime DTBEGINS, string REPORTHEADER, string REPORTHEADERAr, string SELECTEDSETS, string STUDENTCLASS, string SEMESTER, bool INPUT100, string ClassLevel)
	{
		InitializeComponent();
		bot = conv.ArabicToEnglish(BOT);
		mot = conv.ArabicToEnglish(MOT);
		eot = conv.ArabicToEnglish(EOT);
	...
	{
		TermSettings.SetTermSettingsAr(StudentClass, Semester, bot.ToString(), mot.ToString(), eot.ToString(), ProcessAspercentage, dtTermBegins, reportHeader.ToUpper(), reportHeaderAr, _selectedSets, input100);
