...
public class RegisterStudentsNewCur : XtraForm
{
	...
	public RegisterStudentsNewCur(string _Class, string _Term, GridView grvSubjects, GridView grvStudents)
	{
		InitializeComponent();
		this._Class = _Class;
		this._Term = _Term;
	...
	private void RegisterAllOLevel()
	{
		try
		{
			...
			foreach (string text in array)
			{
				...
				{
					string text3 = grvSubjects.GetRowCellValue(selectedRows2[j], "SubjectId").ToString();
					SqlDataAdapter sqlDataAdapter = new SqlDataAdapter($"SELECT * FROM tbl_TermSettingsNC WHERE SemesterId='{text2}' AND ClassId='{_Class}' AND SubjectId='{text3}'", DataConnection.ConnectToDatabase());
					DataTable dataTable = new DataTable();
					sqlDataAdapter.Fill(dataTable);
					if (dataTable.Rows.Count == 0)
					{
			...
			foreach (string text4 in array2)
			{
				...
				for (int l = 0; l < selectedRows.Length; l++)
				{
					...
					{
						string text6 = grvSubjects.GetRowCellValue(selectedRows2[m], "SubjectId").ToString();
						string value = grvStudents.GetRowCellValue(selectedRows[l], "StudentNumber").ToString();
						...
						if (!sqlDataReader.HasRows)
						{
							...
							{
								Connection = sqlConnection2,
								CommandText = "INSERT INTO tbl_Scores_OL_Report (StudentNumber,SemesterId,SubjectId,ClassId,TUnits,IsAssessed, U1,U2,U3,U4,Score1,Score2,Score3,Score4,Hund1,Hund2,Hund3,Hund4,IsEdited) VALUES (@StudentNumber,@SemesterId,@SubjectId,@ClassId,@TUnits,@IsAssessed, @U1,@U2,@U3,@U4,@Score1,@Score2,@Score3,@Score4,@Hund1,@Hund2,@Hund3,@Hund4,@IsEdited)",
								CommandType = CommandType.Text
							})
							{
								...
								sqlParameter2.Value = 1;
								sqlParameter2.Direction = ParameterDirection.Input;
								sqlParameter2 = sqlCommand3.Parameters.Add("@IsEdited", SqlDbType.Bit);
								sqlParameter2.Value = 1;
								sqlParameter2.Direction = ParameterDirection.Input;
