...
using I_Xtreme.AdvancedReports.ReportsMain;
using I_Xtreme.DialogForms;

namespace I_Xtreme;

public class AcademicReport : RibbonForm
{
	private string schoolType = SchoolSettings.SchoolGeneralCurriculum.ToString();

	private MainALevelSET mainALevel = new MainALevelSET();

	private MainPrimary mainPrimary = new MainPrimary();

	private MainALevelSETLandScale mainALevelLS = new MainALevelSETLandScale();
	...
	public AcademicReport(string __Class, string __Semester)
	{
	...
	private void LoadReport()
	{
		try
		{
			...
			{
				if (_Class == "S.5" || _Class == "S.6")
				{
					if (ReportCustomization.IsPotrait)
					{
						printControl1.PrintingSystem = mainALevel.PrintingSystem;
						mainALevel.DisplayName = text;
						mainALevel.Bookmark = text;
						mainALevel.CreateDocument();
					}
					else if (!ReportCustomization.IsPotrait)
					{
						printControl1.PrintingSystem = mainALevelLS.PrintingSystem;
						mainALevelLS.DisplayName = text;
						mainALevelLS.Bookmark = text;
						mainALevelLS.CreateDocument();
					}
				}
			}
			else if (schoolType == SchoolSettings.SchoolType.Primary.ToString())
			{
				printControl1.PrintingSystem = mainPrimary.PrintingSystem;
				mainPrimary.DisplayName = text;
				mainPrimary.CreateDocument();
			}
		}
		catch (Exception ex)
		{
