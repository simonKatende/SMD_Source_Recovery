...
public class usrGuardianWorklist : XtraUserControl
{
    ...
    public usrGuardianWorklist()
    {
        DevExpress.XtraSplashScreen.SplashScreenManager.ShowForm(typeof(I_Xtreme.DialogForms.WaitForm1));
        DevExpress.XtraSplashScreen.SplashScreenManager.Default.SetWaitFormDescription("Loading Guardian Worklist...");
    ...
    private void LoadData()
    {
        try
        {
            ...
            {
                var classes = _allRows.SelectMany(r => r.Students).Select(s => s.ClassId)
        ...
        {
            XtraMessageBox.Show($"Could not load guardian worklist.\n\n{ex.Message}",
    ...
    private void ConfigureColumns()
    {
        ...
        AddCol("PaymentStatus",       "Status",         90);
    ...
    private void TxtSearch_EditValueChanged(object sender, EventArgs e)
    {
        ...
        _grid.DataSource = string.IsNullOrEmpty(q) ? _allRows :
            _allRows.Where(r =>
                ...
                r.Students.Any(st => st.FullName.ToLower().Contains(q) || st.StudentNumber.Contains(q))
            ).ToList();
    }
    ...
    public void Print(bool preview = false)
    {
        ...
        if (preview)
            PrintableControl.PreviewControl("Guardian Worklist", DateTime.Today.ToString("dd-MMM-yyyy"), link, IsLandScape: true, _grid);
        else
            PrintableControl.PrintControl("Guardian Worklist", DateTime.Today.ToString("dd-MMM-yyyy"), link, IsLandScape: true, _grid);
    }
    ...
    public void Export()
    {
        var ps   = new PrintingSystem();
        var link = new PrintableComponentLink(ps);
        PrintableControl.ExportControl("Guardian Worklist", DateTime.Today.ToString("dd-MMM-yyyy"), link, _grid);
    }
}
