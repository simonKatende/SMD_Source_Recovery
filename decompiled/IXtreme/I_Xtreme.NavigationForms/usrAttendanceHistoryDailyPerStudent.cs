...
public class usrAttendanceHistoryDailyPerStudent : XtraUserControl
{
	...
	public usrAttendanceHistoryDailyPerStudent(DateTime Start)
	{
		InitializeComponent();
		this.Start = Start;
	...
	{
		string selectCommandText = string.Format("SELECT        s.fullName, s.StudentNumber, s.StudentID, s.ClassId, s.StreamId, s.Sex, s.Status, a.AttendanceCount, a.AttendanceCategory FROM  tbl_Stud s INNER JOIN  AttendanceSheet a ON s.StudentNumber = a.StudentNo WHERE(s.Status = 'Active') AND(CONVERT(VARCHAR(10), a.AttendanceDate, 103) = '{0}') AND (a.AttendanceCategory <> N'Reporting')", Start.ToString("dd/MM/yyyy"));
	...
	private void InitializeComponent()
	{
		...
		base.Name = "usrAttendanceHistoryDailyPerStudent";
