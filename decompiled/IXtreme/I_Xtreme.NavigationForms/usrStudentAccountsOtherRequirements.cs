...
public class usrStudentAccountsOtherRequirements : XtraUserControl
{
	private int hotTrackRow = int.MinValue;

	private string particulars = string.Empty;
	...
	private DataTable FeesAndOtherRequirement
	{
		get
		{
			...
			{
				using SqlDataAdapter sqlDataAdapter2 = new SqlDataAdapter(string.Format("SELECT (ISNULL(SUM(Debit),0)-ISNULL(SUM(Credit),0)) AS PreviousBalance FROM FeesPayment GROUP BY StudentNumber,SemesterId,Particulars  HAVING (StudentNumber = '{0}') AND (SemesterId = '{1}') AND Particulars='{2}'", row["StudentNo"], GetPreviousSemester(StudentOptions.ActiveSemester()), particulars), DataConnection.ConnectToDatabase());
				...
				if (dataTable3.Rows.Count > 0)
				{
					...
					{
						string selectCommandText = string.Format("SELECT ISNULL(SUM(Debit),0) AS Required,ISNULL(SUM(Credit),0) AS Paid FROM FeesPayment GROUP BY StudentNumber, SemesterId,Particulars  HAVING (StudentNumber = '{0}') AND (SemesterId = '{1}') AND Particulars='{2}'", row["StudentNo"], StudentOptions.ActiveSemester(), particulars);
				...
				else
				{
					...
					string selectCommandText2 = string.Format("SELECT StudentNumber, SemesterId, SUM(Debit) AS Required, SUM(Credit) AS Paid FROM FeesPayment GROUP BY StudentNumber, SemesterId,Particulars HAVING (StudentNumber = '{0}') AND (SemesterId = '{1}') AND Particulars='{2}'", row["StudentNo"], StudentOptions.ActiveSemester(), particulars);
	...
	public usrStudentAccountsOtherRequirements(string particulars)
	{
		SplashScreenManager.ShowForm(typeof(WaitForm1));
		SplashScreenManager.Default.SetWaitFormDescription("Loading " + StudentOptions.ActiveClass() + " " + particulars + " Tracking Sheet...");
	...
	private void LoadStudentLists()
	{
		try
		{
			...
			ActiveFormSelected.SetActiveForm(StudentOptions.ActiveClass() + " " + particulars + " Tracking Sheet");
	...
	{
		SplashScreenManager.ShowForm(typeof(WaitForm1));
		SplashScreenManager.Default.SetWaitFormDescription("Refreshing " + StudentOptions.ActiveClass() + " " + particulars + " Tracking Sheet...");
		SplashScreenManager.Default.SendCommand(WaitForm1.WaitFormCommand.InitializeFeesTrackingSheet, 0);
		Thread.Sleep(25);
		LoadStudentLists();
		SplashScreenManager.CloseForm(throwExceptionIfAlreadyClosed: false);
	...
	private void InitializeComponent()
	{
		...
		base.Name = "usrStudentAccountsOtherRequirements";
