...
public class XtraUserIssueHistory : XtraUserControl
{
	...
	public XtraUserIssueHistory(DateTime sdate, DateTime edate)
	{
		InitializeComponent();
		this.sdate = sdate;
		this.edate = edate;
	...
	private void LoadIssueHistory()
	{
		try
		{
			...
			using SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(string.Format("SELECT  o.itemId, o.auto, o.Issued AS qty,ISNULL(o.UPP,0) AS Rate,(ISNULL(o.UPP,0)*ISNULL(o.Issued,0)) AS Total, o.OldStock, s.AssetName, w._Date, w.Status, w.Notes, w.DispTo, w.Mob FROM tbl_WasteDispenseDetails o INNER JOIN  tbl_StockItems s ON s.id = o.itemId INNER JOIN tbl_WasteDispense w ON o.DispID = w.DispID WHERE (o.JobType = 'Issue') AND (w.Status = 'Closed') AND (w._Date >= '{0}' AND w._Date <= '{1}')", sdate.ToString("MM/dd/yyyy"), edate.ToString("MM/dd/yyyy")), selectConnection);
			...
			ActiveFormSelected.SetActiveForm(string.Format("Dispense History Report: {0}-{1}", sdate.ToString("dd-MMM-yy"), edate.ToString("dd-MMM-yy")));
		}
		catch (Exception ex)
		{
	...
	private void InitializeComponent()
	{
		...
		base.Name = "XtraUserIssueHistory";
