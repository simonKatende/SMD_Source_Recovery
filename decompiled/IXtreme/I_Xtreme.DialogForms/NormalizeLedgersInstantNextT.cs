...
public class NormalizeLedgersInstantNextT : XtraForm
{
	...
	public NormalizeLedgersInstantNextT(string _Class)
	{
		InitializeComponent();
		this._Class = _Class;
	}
	...
	private void UpdateFeesBalance()
	{
		try
		{
			...
			foreach (DataRow row in dataSet.Tables[0].Rows)
			{
				...
				SqlDataAdapter sqlDataAdapter2 = new SqlDataAdapter(string.Format("SELECT PaymentId,Debit, Credit,(Debit-Credit) AS Amount FROM FeesPaymentNextTerm WHERE StudentNumber='{0}' ORDER BY PaymentId", row["StudentNumber"]), DataConnection.ConnectToDatabase());
				...
				for (int i = 0; i < myGridView1.RowCount; i++)
				{
					...
					using (SqlCommand sqlCommand = new SqlCommand
					{
						...
						CommandText = "UPDATE FeesPaymentNextTerm SET Balance=@Balance WHERE PaymentId=@PaymentId AND StudentNumber=@StudentNumber",
