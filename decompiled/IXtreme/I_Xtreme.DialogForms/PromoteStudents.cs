...
public class PromoteStudents : XtraForm
{
	...
	private BandedGridColumn bandedGridColClass;

	private BandedGridColumn bandedGridColPromote;

	private BandedGridColumn bandedGridColPOB;

	private BandedGridColumn bandedGridColRepeat;

	private BandedGridColumn bandedGridColumnPromote;

	private BandedGridColumn bandedGridColumnPOB;

	private BandedGridColumn bandedGridColumnRepeat;
	...
	private void TransferCandidates()
	{
		try
		{
			...
			array = new string[2] { "S.6", "S.4" };
			string text = DataConnection.ConnectToDatabase();
			for (int i = 0; i < array.Length; i++)
			{
				currentTask = "Backing up " + array[i] + " students...";
				SqlDataAdapter sqlDataAdapter = new SqlDataAdapter($"SELECT * FROM tbl_Stud  WHERE ClassId='{array[i]}'", text);
				DataSet dataSet = new DataSet();
				...
				foreach (DataRow row in dataSet.Tables[0].Rows)
				{
					...
					using (SqlCommand sqlCommand = new SqlCommand
					{
						...
						Connection = sqlConnection,
						CommandText = "INSERT INTO tbl_oldStudents (StudentNumber,HouseId,StreamId,Oid,FormerSchool,StudyStatus,BursaryStatus,RequiredFees,BursaryProvider,AdmissionDate,Sex,Picture,Religion,HomeCountry,fullName,Status,Notes,cashOnAccount,ClassId,ExitYear,PriorityContact,OtherContact,StudentID,LIN)VALUES(@StudentNumber,@HouseId,@StreamId,@Oid,@FormerSchool,@StudyStatus,@BursaryStatus,@RequiredFees,@BursaryProvider,@AdmissionDate,@Sex,@Picture,@Religion,@HomeCountry,@fullName,@Status,@Notes,@cashOnAccount,@ClassId,@ExitYear,@PriorityContact,@OtherContact,@StudentID,@LIN)",
						CommandType = CommandType.Text
					})
					{
						...
						sqlParameter.Value = row["PriorityContact"];
						sqlParameter.Direction = ParameterDirection.Input;
						sqlParameter = sqlCommand.Parameters.Add("@OtherContact", SqlDbType.VarChar, 11);
						sqlParameter.Value = row["OtherContact"];
						sqlParameter.Direction = ParameterDirection.Input;
						sqlParameter = sqlCommand.Parameters.Add("@StudentID", SqlDbType.VarChar, 12);
						sqlParameter.Value = row["StudentID"];
						sqlParameter.Direction = ParameterDirection.Input;
						sqlParameter = sqlCommand.Parameters.Add("@LIN", SqlDbType.VarChar, 50);
						sqlParameter.Value = row["LIN"];
						sqlParameter.Direction = ParameterDirection.Input;
					...
					using (SqlCommand sqlCommand3 = new SqlCommand
					{
						Transaction = transaction,
						Connection = sqlConnection,
						CommandText = string.Format("DELETE FROM tbl_Scores_AL WHERE StudentNumber='{0}'", row["StudentNumber"].ToString()),
						CommandType = CommandType.Text
					})
					{
					...
					using (SqlCommand sqlCommand4 = new SqlCommand
					{
						Transaction = transaction,
						Connection = sqlConnection,
						CommandText = string.Format("DELETE FROM tbl_GeneralReport_Grading_AL WHERE StudentNumber='{0}'", row["StudentNumber"].ToString()),
						CommandType = CommandType.Text
					})
					{
					...
					using (SqlCommand sqlCommand5 = new SqlCommand
					{
						Transaction = transaction,
						Connection = sqlConnection,
						CommandText = string.Format("DELETE FROM tbl_GeneralReport_AL WHERE StudentNumber='{0}'", row["StudentNumber"].ToString()),
						CommandType = CommandType.Text
					})
					{
					...
					using (SqlCommand sqlCommand6 = new SqlCommand
					{
						Transaction = transaction,
						Connection = sqlConnection,
						CommandText = string.Format("DELETE FROM tbl_ALevelReport WHERE StudentNumber='{0}'", row["StudentNumber"].ToString()),
						CommandType = CommandType.Text
					})
					{
	...
	private void PromoteStudentsUnConditionally()
	{
		try
		{
			string[] array = new string[4] { "S.5", "S.3", "S.2", "S.1" };
			for (int i = 0; i < array.Length; i++)
			{
				string value = "S." + (Convert.ToInt32(array[i].ToString().Substring(2, 1)) + 1);
				currentTask_Promote = "Promoting " + array[i] + " students...";
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(I_Xtreme.DialogForms.PromoteStudents));
		...
		this.bandedGridColClass = new DevExpress.XtraGrid.Views.BandedGrid.BandedGridColumn();
		this.gridBand1 = new DevExpress.XtraGrid.Views.BandedGrid.GridBand();
		this.bandedGridColPromote = new DevExpress.XtraGrid.Views.BandedGrid.BandedGridColumn();
		this.bandedGridColPOB = new DevExpress.XtraGrid.Views.BandedGrid.BandedGridColumn();
		this.bandedGridColRepeat = new DevExpress.XtraGrid.Views.BandedGrid.BandedGridColumn();
		this.gridBand2 = new DevExpress.XtraGrid.Views.BandedGrid.GridBand();
		this.bandedGridColumnPromote = new DevExpress.XtraGrid.Views.BandedGrid.BandedGridColumn();
		this.bandedGridColumnPOB = new DevExpress.XtraGrid.Views.BandedGrid.BandedGridColumn();
		this.bandedGridColumnRepeat = new DevExpress.XtraGrid.Views.BandedGrid.BandedGridColumn();
		...
		this.gridControl1.ViewCollection.AddRange(new DevExpress.XtraGrid.Views.Base.BaseView[1] { this.bandedGridView1 });
		this.bandedGridView1.Bands.AddRange(new DevExpress.XtraGrid.Views.BandedGrid.GridBand[3] { this.gridBand3, this.gridBand1, this.gridBand2 });
		this.bandedGridView1.Columns.AddRange(new DevExpress.XtraGrid.Views.BandedGrid.BandedGridColumn[7] { this.bandedGridColClass, this.bandedGridColPromote, this.bandedGridColPOB, this.bandedGridColRepeat, this.bandedGridColumnPromote, this.bandedGridColumnPOB, this.bandedGridColumnRepeat });
		this.bandedGridView1.GridControl = this.gridControl1;
		this.bandedGridView1.Name = "bandedGridView1";
		this.bandedGridView1.OptionsView.ShowGroupPanel = false;
		this.bandedGridView1.OptionsView.ShowIndicator = false;
		this.gridBand3.Caption = "Class";
		this.gridBand3.Columns.Add(this.bandedGridColClass);
		...
		this.gridBand1.Caption = "O Level";
		this.gridBand1.Columns.Add(this.bandedGridColPromote);
		this.gridBand1.Columns.Add(this.bandedGridColPOB);
		this.gridBand1.Columns.Add(this.bandedGridColRepeat);
		...
		this.gridBand2.Caption = "A Level";
		this.gridBand2.Columns.Add(this.bandedGridColumnPromote);
		this.gridBand2.Columns.Add(this.bandedGridColumnPOB);
		this.gridBand2.Columns.Add(this.bandedGridColumnRepeat);
		this.gridBand2.Name = "gridBand2";
		this.gridBand2.VisibleIndex = 2;
		this.gridBand2.Width = 478;
		this.bandedGridColumnPromote.Caption = "Promote";
		this.bandedGridColumnPromote.Name = "bandedGridColumnPromote";
		this.bandedGridColumnPromote.Visible = true;
		this.bandedGridColumnPromote.Width = 155;
		this.bandedGridColumnPOB.Caption = "POB";
		this.bandedGridColumnPOB.Name = "bandedGridColumnPOB";
		this.bandedGridColumnPOB.Visible = true;
		this.bandedGridColumnPOB.Width = 155;
		this.bandedGridColumnRepeat.Caption = "Repeat";
		this.bandedGridColumnRepeat.Name = "bandedGridColumnRepeat";
		this.bandedGridColumnRepeat.Visible = true;
		this.bandedGridColumnRepeat.Width = 168;
		...
		this.lblDescription.Size = new System.Drawing.Size(496, 13);
		this.lblDescription.TabIndex = 2;
		this.lblDescription.Text = "O Level students will be promoted basing on Average Score and A Level students on total points scored";
		...
		{
			new DevExpress.XtraEditors.Controls.RadioGroupItem(null, "Promote all students unconditionally"),
			new DevExpress.XtraEditors.Controls.RadioGroupItem(null, "Promote students basing on the following conditions")
		});
		...
		this.completionWizardPage1.FinishText = "You have successfully promoted the students";
