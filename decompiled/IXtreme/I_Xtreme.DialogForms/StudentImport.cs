...
public class StudentImport : XtraForm
{
	private int k = 0;
	...
	private string schoolType = SchoolSettings.SchoolGeneralCurriculum;
	...
	private void InsertToSql()
	{
		try
		{
			...
			progressBarClass.Properties.Maximum = num;
			string[] array2 = Classes.ListOfClasses().Split();
			foreach (string text2 in array2)
			{
				lblCurrentClass.Text = "Processing " + text2 + " class.";
				k = 0;
				...
				{
					using OleDbDataAdapter oleDbDataAdapter = new OleDbDataAdapter($"select * from [{listBoxControl1.SelectedItem.ToString()}$] WHERE Class='{text2}'", selectConnection);
					...
					{
						string text3 = StudentNumber.GetNextStudentNumber()[0];
						if (!StudentNumber.StudentNumberExists(text3))
						{
							System.Windows.Forms.Application.DoEvents();
							lblCurrentStudent.Text = string.Format("{0}", Thread.CurrentThread.CurrentCulture.TextInfo.ToTitleCase(row["Name"].ToString()));
							Guid guid = Guid.NewGuid();
							Image @default = Resources.Default;
							int num2 = @default.Height;
							int num3 = @default.Width;
							num2 = num2 * 220 / num3;
							num3 = 220;
							if (num2 > 250)
							{
							...
							Bitmap bitmap = new Bitmap(@default, num3, num2);
							MemoryStream memoryStream = new MemoryStream();
							bitmap.Save(memoryStream, ImageFormat.Png);
							memoryStream.Position = 0L;
							byte[] array3 = new byte[memoryStream.Length + 1];
							...
							using (SqlConnection sqlConnection = new SqlConnection(DataConnection.ConnectToDatabase()))
							{
								...
								using (SqlCommand sqlCommand = new SqlCommand
								{
									...
									Transaction = trans,
									CommandText = "INSERT INTO tbl_Stud (fullName,StudentId,Oid,StudentNumber,ClassId,StreamId,Sex,StudyStatus,HouseId,Religion,Guardian,BursaryStatus,cashOnAccount,RequiredFees,AdmissionDate,Picture,HomeCountry,Status)values(@fullName,@StudentId, @Oid, @StudentNumber,@ClassId,@StreamId,@Sex,@StudyStatus,@HouseId,@Religion,@Guardian,@BursaryStatus,@cashOnAccount,@RequiredFees,@AdmissionDate,@Picture,@HomeCountry,@Status)",
								...
								})
								{
									...
									sqlParameter.Value = Thread.CurrentThread.CurrentCulture.TextInfo.ToTitleCase(row["Name"].ToString());
									sqlParameter.Direction = ParameterDirection.Input;
									sqlParameter = sqlCommand.Parameters.Add("@StudentId", SqlDbType.VarChar, 50);
									sqlParameter.Value = Thread.CurrentThread.CurrentCulture.TextInfo.ToTitleCase(row["StudentId"].ToString());
									sqlParameter.Direction = ParameterDirection.Input;
									...
									sqlParameter.Value = row["Stream"].ToString().TrimStart().TrimEnd();
									sqlParameter.Direction = ParameterDirection.Input;
									sqlParameter = sqlCommand.Parameters.Add("@sex", SqlDbType.VarChar, 1);
									sqlParameter.Value = row["Sex"].ToString().TrimStart().TrimEnd();
									sqlParameter.Direction = ParameterDirection.Input;
									...
									sqlParameter.Value = row["House"].ToString();
									sqlParameter.Direction = ParameterDirection.Input;
									...
									sqlParameter.Value = FeesMapping.RequriedFees(text2, row["studyStatus"].ToString());
									sqlParameter.Direction = ParameterDirection.Input;
								...
								using (SqlCommand sqlCommand2 = new SqlCommand
								{
									Connection = sqlConnection,
									Transaction = trans,
									CommandText = "INSERT INTO tbl_Guardian (studentOid) VALUES (@studentOid)",
									CommandType = CommandType.Text
								})
								{
							...
							System.Windows.Forms.Application.DoEvents();
							WriteProgress(string.Format("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>Committed</td></tr>", row["Name"], text3, text2));
							k++;
							progressBarStudents.Position = k;
						}
						else
						{
							WriteProgress(string.Format("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>Skipped</td></tr>", row["Name"], text3, text2));
						}
					}
					if (k == dataTable.Rows.Count)
					{
						lblCurrentClass.Text = string.Empty;
						lblCurrentStudent.Text = "Complete.";
					}
				}
				System.Windows.Forms.Application.DoEvents();
				this.i++;
				progressBarClass.Position = this.i;
			}
			WriteProgress("</table>");
	...
	private void ValidateColumns()
	{
		...
		if (gridView1.Columns.Contains(gridView1.Columns["StudentID"]))
		{
			dtStudentID = new DataTable();
			dtStudentID.Columns.Add("#", typeof(int));
			dtStudentID.Columns.Add("StudentID", typeof(string));
			bool flag2 = false;
			int num = 0;
			for (int i = 0; i < gridView1.RowCount; i++)
			{
				if (gridView1.GetRowCellValue(i, "StudentID").ToString().Length == 0 || gridView1.GetRowCellValue(i, "StudentID").ToString().Length > 12)
				{
					flag = true;
					flag2 = true;
					num++;
					dtStudentID.Rows.Add(num, gridView1.GetRowCellValue(i, "StudentID").ToString());
				}
			}
			if (!flag2)
			{
				dataTable.Rows.Add("StudentID", Resources.columnOK, "OK");
			}
			else
			{
				flag = true;
				dataTable.Rows.Add("StudentID", Resources.columnWarning, num + " errors(s) found. (StudentID cannot be blank or more than 12 characters)");
			}
		}
		else
		{
			dataTable.Rows.Add("StudentID", Resources.columnNone, "Column Not Found");
		}
		if (gridView1.Columns.Contains(gridView1.Columns["Name"]))
		{
			dataTable.Rows.Add("Name", Resources.columnOK, "OK");
		}
		else
		{
			flag = true;
			dataTable.Rows.Add("Name", Resources.columnNone, "Column Not Found");
		}
		...
		if (gridView1.Columns.Contains(gridView1.Columns["Class"]))
		{
			...
			else if (schoolType == "Primary")
			{
				...
				{
					if (gridView1.GetRowCellValue(m, "Class").ToString() != "P.1" && gridView1.GetRowCellValue(m, "Class").ToString() != "P.2" && gridView1.GetRowCellValue(m, "Class").ToString() != "P.3" && gridView1.GetRowCellValue(m, "Class").ToString() != "P.4" && gridView1.GetRowCellValue(m, "Class").ToString() != "P.5" && gridView1.GetRowCellValue(m, "Class").ToString() != "P.6" && gridView1.GetRowCellValue(m, "Class").ToString() != "P.7" && gridView1.GetRowCellValue(m, "Class").ToString() != "Baby" && gridView1.GetRowCellValue(m, "Class").ToString() != "Middle" && gridView1.GetRowCellValue(m, "Class").ToString() != "Top" && gridView1.GetRowCellValue(m, "Class").ToString() != "Day-Care")
					{
						flag = true;
						flag6 = true;
						num5++;
						dtClass.Rows.Add(num5, gridView1.GetRowCellValue(m, "Class").ToString());
					}
				}
				if (!flag6)
				{
				...
				{
					flag = true;
					dataTable.Rows.Add("Class", Resources.columnWarning, num5 + " errors(s) found. (Expected Data Input: Day-Care,Baby, Middle, Top, P.1, P.2, P.3, P.4, P.5, P.6 OR P.7)");
				}
			}
		}
		else
	...
	private void gridView2_RowClick(object sender, RowClickEventArgs e)
	{
		if (gridView2.GetRowCellValue(gridView2.FocusedRowHandle, "ColumnName").ToString() == "StudyStatus" && gridView2.GetRowCellValue(gridView2.FocusedRowHandle, "Description").ToString() != "OK" && gridView2.GetRowCellValue(gridView2.FocusedRowHandle, "Description").ToString() != "Column Not Found")
		{
			...
			{
				errorDescription.gridControl1.DataSource = dtStudyStatus.DefaultView;
				errorDescription.ShowDialog();
				return;
			}
		}
		if (gridView2.GetRowCellValue(gridView2.FocusedRowHandle, "ColumnName").ToString() == "StudentID" && gridView2.GetRowCellValue(gridView2.FocusedRowHandle, "Description").ToString() != "OK" && gridView2.GetRowCellValue(gridView2.FocusedRowHandle, "Description").ToString() != "Column Not Found")
		{
			using (ErrorDescription errorDescription2 = new ErrorDescription())
			{
				errorDescription2.gridControl1.DataSource = dtStudentID.DefaultView;
	...
	private void InitializeComponent()
	{
		...
		this.wizardControl1.HeaderImageOptions.Image = I_Xtreme.Properties.Resources.importStud2;
		this.wizardControl1.ImageOptions.Image = I_Xtreme.Properties.Resources.importStud;
		...
		this.welcomeWizardPage1.Size = new System.Drawing.Size(455, 369);
		...
		this.wizardPage1.Size = new System.Drawing.Size(640, 358);
		...
		this.labelControl5.Location = new System.Drawing.Point(101, 174);
		...
		this.labelControl4.Location = new System.Drawing.Point(101, 159);
		...
		this.labelControl10.Size = new System.Drawing.Size(63, 97);
		this.labelControl10.TabIndex = 45;
		this.labelControl10.Text = "StudentID\r\nName\r\nClass\r\nStream\r\nSex\r\nStudyStatus\r\nHouse\r\n";
		...
		this.completionWizardPage1.Size = new System.Drawing.Size(455, 369);
		...
		this.checkEdit1.Size = new System.Drawing.Size(432, 20);
		...
		this.wizardPage2.Size = new System.Drawing.Size(640, 358);
		...
		this.layoutControl1.Size = new System.Drawing.Size(640, 358);
		this.layoutControl1.TabIndex = 0;
		this.layoutControl1.Text = "layoutControl1";
		this.listBoxControl1.Location = new System.Drawing.Point(2, 2);
		this.listBoxControl1.Name = "listBoxControl1";
		this.listBoxControl1.Size = new System.Drawing.Size(113, 354);
		...
		this.gridControl1.Size = new System.Drawing.Size(519, 354);
		...
		this.layoutControlGroup1.Size = new System.Drawing.Size(640, 358);
		this.layoutControlGroup1.TextVisible = false;
		this.layoutControlItem1.Control = this.gridControl1;
		this.layoutControlItem1.CustomizationFormText = "layoutControlItem1";
		this.layoutControlItem1.Location = new System.Drawing.Point(117, 0);
		this.layoutControlItem1.Name = "layoutControlItem1";
		this.layoutControlItem1.Size = new System.Drawing.Size(523, 358);
		...
		this.layoutControlItem2.Size = new System.Drawing.Size(117, 358);
		this.layoutControlItem2.TextSize = new System.Drawing.Size(0, 0);
		this.layoutControlItem2.TextVisible = false;
		this.wizardPage4.Controls.Add(this.labelControl3);
		...
		this.wizardPage4.Size = new System.Drawing.Size(640, 358);
		...
		this.labelControl3.Location = new System.Drawing.Point(20, 230);
		this.labelControl3.Name = "labelControl3";
		this.labelControl3.Size = new System.Drawing.Size(67, 26);
		this.labelControl3.TabIndex = 4;
		this.labelControl3.Text = "Importing:";
		this.lblCurrentClass.Appearance.Options.UseTextOptions = true;
		...
		this.lblCurrentStudent.Appearance.Font = new System.Drawing.Font("Tahoma", 9.75f, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, 0);
		this.lblCurrentStudent.Appearance.Options.UseFont = true;
		this.lblCurrentStudent.Appearance.Options.UseTextOptions = true;
		this.lblCurrentStudent.Appearance.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Near;
		this.lblCurrentStudent.AutoSizeMode = DevExpress.XtraEditors.LabelAutoSizeMode.None;
		this.lblCurrentStudent.Location = new System.Drawing.Point(93, 230);
		this.lblCurrentStudent.Name = "lblCurrentStudent";
		this.lblCurrentStudent.Size = new System.Drawing.Size(525, 26);
		...
		this.wizardPage5.Size = new System.Drawing.Size(640, 358);
		...
		this.lblDescriptionStatus.Appearance.Options.UseFont = true;
		...
		this.gridControl2.Size = new System.Drawing.Size(640, 255);
