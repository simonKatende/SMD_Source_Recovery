...
public class SubjectTermUnitsTH : XtraForm
{
	...
	{
		SqlDataAdapter sqlDataAdapter = new SqlDataAdapter($"SELECT * FROM tbl_TermSettingsNCTH   WHERE SemesterId='{cboTerm.Text}' AND ClassId='{cboClass.Text}'", DataConnection.ConnectToDatabase());
	...
	private string UpdateCommand(int UnitsUsed)
	{
		...
		{
		case 0:
			result = "UPDATE tbl_Scores_OL_ReportTH SET U1='x',U2='x',U3='x',U4='x', Score1='x',Score2='x',Score3='x',Score4='x', Hund1='x',Hund2='x',Hund3='x',Hund4='x',TUnits=0 WHERE ClassId=@ClassId AND SubjectId=@SubjectId AND SemesterId=@SemesterId";
			break;
		case 1:
			result = "UPDATE tbl_Scores_OL_ReportTH SET U2='x',U3='x',U4='x',Score2='x',Score3='x',Score4='x',Hund2='x',Hund3='x',Hund4='x', TUnits=1 WHERE ClassId=@ClassId AND SubjectId=@SubjectId AND SemesterId=@SemesterId";
			break;
		case 2:
			result = "UPDATE tbl_Scores_OL_ReportTH SET U3='x',U4='x',Score3='x',Score4='x',Hund3='x',Hund4='x', TUnits=2 WHERE ClassId=@ClassId AND SubjectId=@SubjectId AND SemesterId=@SemesterId";
			break;
		case 3:
			result = "UPDATE tbl_Scores_OL_ReportTH SET U4='x',Score4='x', Hund4='x', TUnits=3 WHERE ClassId=@ClassId AND SubjectId=@SubjectId AND SemesterId=@SemesterId";
			break;
		case 4:
			result = "UPDATE tbl_Scores_OL_ReportTH SET TUnits=4 WHERE ClassId=@ClassId AND SubjectId=@SubjectId AND SemesterId=@SemesterId";
			break;
		}
		return result;
	}
	...
	private void UpdateUnitsDown(CellValueChangedEventArgs e)
	{
		...
		using (SqlCommand sqlCommand = new SqlCommand
		{
			...
			Transaction = trans,
			CommandText = "UPDATE tbl_TermSettingsNCTH SET TUnits=@TUnits WHERE SubjectId=@SubjectId AND SemesterId=@SemesterId AND ClassId=@ClassId",
	...
	private void UpdateUnitsUp(CellValueChangedEventArgs e)
	{
		...
		{
			string commandText = string.Format("UPDATE tbl_Scores_OL_ReportTH SET U{0}='',Score{0}='',Hund{0}='', TUnits={1} WHERE ClassId=@ClassId AND SubjectId=@SubjectId AND SemesterId=@SemesterId", i, newValue);
			using SqlCommand sqlCommand2 = new SqlCommand
			{
				Connection = sqlConnection,
				CommandText = commandText,
	...
	private void gridView1_CellValueChanged(object sender, CellValueChangedEventArgs e)
	{
		...
		else if (e.Column.FieldName == "IsAssessed")
		{
			...
			{
				Connection = sqlConnection,
				CommandText = "UPDATE tbl_Scores_OL_ReportTH SET IsAssessed=@IsAssessed WHERE ClassId=@ClassId AND SubjectId=@SubjectId AND SemesterId=@SemesterId",
				CommandType = CommandType.Text
			};
		...
		else if (e.Column.FieldName == "ProjectAvailable")
		{
			...
			SqlCommand sqlCommand2 = new SqlCommand
			{
				...
				CommandText = "UPDATE tbl_TermSettingsNCTH SET ProjectAvailable=@ProjectAvailable WHERE ClassId=@ClassId AND SubjectId=@SubjectId AND SemesterId=@SemesterId",
				CommandType = CommandType.Text
			};
	...
	private void gridView1_ValidatingEditor(object sender, BaseContainerValidateEditorEventArgs e)
	{
		...
		{
			if (!int.TryParse(e.Value.ToString(), out var result) || result > 4 || result < 1)
			{
				e.Valid = false;
			}
	...
	private void gridView1_InvalidValueException(object sender, InvalidValueExceptionEventArgs e)
	{
		...
		e.ErrorText = "You have to enter digits ranging from 1 to 4 only.";
		gridView1.HideEditor();
	}
