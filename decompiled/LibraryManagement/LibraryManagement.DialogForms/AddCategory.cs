...
public class AddCategory : XtraForm
{
	...
	private void AddBookCategories()
	{
		try
		{
			...
			using SqlCommand sqlCommand = new SqlCommand
			{
				...
				CommandText = "SELECT * FROM tbl_BookCategories WHERE Category=@Category",
			...
			if (!sqlDataReader.HasRows)
			{
				...
				using SqlCommand sqlCommand2 = new SqlCommand
				{
					...
					CommandText = "INSERT INTO tbl_BookCategories (Category) VALUES (@Category)",
					CommandType = CommandType.Text
				};
			...
			sqlConnection.Close();
			XtraMessageBox.Show("Sorry! You cannot to that,the category you are adding already exists.", "Duplicate Entry Detected", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
		}
		catch (Exception ex)
		{
	...
	private void UpdateCategory()
	{
		...
		{
			Connection = sqlConnection,
			CommandText = "UPDATE tbl_BookCategories SET Category = @Category WHERE CatId=@CatId",
			CommandType = CommandType.Text
		};
		SqlParameter sqlParameter = sqlCommand.Parameters.Add("@CatId", SqlDbType.BigInt);
		sqlParameter.Value = catID;
		sqlParameter.Direction = ParameterDirection.Input;
	...
	private void AddCategory_Load(object sender, EventArgs e)
	{
		if (Text.Contains("Add"))
		{
			simpleButton2.Text = "Add Category";
			simpleButton1.Text = "Close";
		}
		else if (Text.Contains("Edit"))
		{
			simpleButton2.Text = "Update Category";
	...
	private void InitializeComponent()
	{
		...
		this.txtCategory.Location = new System.Drawing.Point(56, 4);
		this.txtCategory.Name = "txtCategory";
		this.txtCategory.Properties.BorderStyle = DevExpress.XtraEditors.Controls.BorderStyles.HotFlat;
		...
		base.Load += new System.EventHandler(AddCategory_Load);
