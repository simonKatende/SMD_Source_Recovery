...
namespace AlienAge.TermlySettings.Thematic.Secondary.OLevel;

public class OLevelProcessProgress : XtraForm
{
	...
	private string generalReportGrading = "";
	...
	public OLevelProcessProgress(double HOP, double BOT, double MOT, double EOT, bool PROCESSASPER, DateTime DTBEGINS, string REPORTHEADER, string SELECTEDSETS, string STUDENTCLASS, string SEMESTER, bool INPUT100, string H_HoP, string H_BOT, string H_MOT, string H_EOT)
	{
		InitializeComponent();
		scoresTable = SchoolSettings.ScoresTableSource(STUDENTCLASS);
		generalReport = SchoolSettings.GeneralReportTableSource(STUDENTCLASS);
		generalReportGrading = SchoolSettings.GeneralReportGradingTableSource(STUDENTCLASS);
		hop = HOP;
		bot = BOT;
		mot = MOT;
		eot = EOT;
	...
	private void UpdateScores(string StudentNumber, string Subject, string AvMark, string Category, string Grade, string Comment, string HOP, string BOT, string MOT, string EOT)
	{
		try
		{
			...
			string commandText = $"UPDATE {generalReport} SET HoP=@HoP,BOT=@BOT,MOT=@MOT,EOT=@EOT,AvMark = @AvMark,Grade = @Grade,Category = @Category,Comment = @Comment WHERE StudentNumber = @StudentNumber AND SubjectId = @SubjectId AND SemesterId = @SemesterId AND ClassId = @ClassId";
	...
	private void ProcessOLevelReportCards()
	{
		try
		{
			using SqlConnection selectConnection = new SqlConnection(connectionString);
			SqlDataAdapter sqlDataAdapter = new SqlDataAdapter($"SELECT * FROM {scoresTable} WHERE ClassId = '{StudentClass}' AND SemesterId='{Semester}'", selectConnection);
				...
				else
				{
					foreach (DataRow row2 in OLevelGradingScale.Instance.SubjectGradingScale.Rows)
					{
						...
						if (ProcessAspercentage)
						{
							...
							num6 = (num + num2 + num3 + num4) / (double)num7;
							num5 = Math.Round(num6, 0);
						}
