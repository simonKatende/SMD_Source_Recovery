...
public class RegisterStudentsNewCur : XtraForm
{
	private int i;
	...
	public RegisterStudentsNewCur(string _Class, string _Term, GridView grvStudents, string subject, int UnitsUsed)
	{
		InitializeComponent();
		this._Class = _Class;
		this._Term = _Term;
	...
	private void RegisterAllOLevel()
	{
		try
		{
			...
			SqlDataAdapter sqlDataAdapter = new SqlDataAdapter($"SELECT * FROM tbl_TermSettingsNC WHERE SemesterId='{_Term}' AND ClassId='{_Class}' AND SubjectId='{subject}'", DataConnection.ConnectToDatabase());
			...
			{
				string value = grvStudents.GetRowCellValue(selectedRows[i], "StudentNumber").ToString();
				...
				{
					sqlCommand2.Parameters.AddWithValue("@StudentNumber", value);
					sqlCommand2.Parameters.AddWithValue("@SemesterId", _Term);
					sqlCommand2.Parameters.AddWithValue("@SubjectId", subject);
					...
					if (!sqlDataReader.HasRows)
					{
						...
						{
							Connection = sqlConnection2,
							CommandText = "INSERT INTO tbl_Scores_OL_Report (StudentNumber,SemesterId,SubjectId,ClassId,TUnits,IsAssessed, U1,U2,U3,U4,Score1,Score2,Score3,Score4,Hund1,Hund2,Hund3,Hund4) VALUES (@StudentNumber,@SemesterId,@SubjectId,@ClassId,@TUnits,@IsAssessed, @U1,@U2,@U3,@U4,@Score1,@Score2,@Score3,@Score4,@Hund1,@Hund2,@Hund3,@Hund4)",
							CommandType = CommandType.Text
						})
						{
			...
			string selectCommandText = $"SELECT StudentNumber,SemesterId,ClassId FROM tbl_Scores_OL_Report WHERE ClassId='{_Class}' GROUP BY StudentNumber,SemesterId,ClassId";
			SqlDataAdapter sqlDataAdapter2 = new SqlDataAdapter(selectCommandText, connectionString);
			...
			foreach (DataRow row in dataTable2.Rows)
			{
				using (SqlConnection sqlConnection3 = new SqlConnection(connectionString))
				{
					...
					using SqlCommand sqlCommand4 = new SqlCommand
					{
						...
						CommandText = "SELECT * FROM tbl_GeneralReport_OL WHERE StudentNumber=@StudentNumber AND SemesterId=@SemesterId AND ClassId=@ClassId",
						CommandType = CommandType.Text
					};
					sqlCommand4.Parameters.AddWithValue("@StudentNumber", row["StudentNumber"].ToString());
					sqlCommand4.Parameters.AddWithValue("@SemesterId", _Term);
					sqlCommand4.Parameters.AddWithValue("@ClassId", _Class);
					...
					if (!sqlDataReader2.HasRows)
					{
						...
						using SqlCommand sqlCommand5 = new SqlCommand
						{
							...
							CommandText = "INSERT INTO tbl_GeneralReport_OL (StudentNumber,SemesterId,ClassId) VALUES (@StudentNumber,@SemesterId,@ClassId)",
							CommandType = CommandType.Text
						};
						SqlParameter sqlParameter3 = sqlCommand5.Parameters.Add("@StudentNumber", SqlDbType.VarChar, 12);
						sqlParameter3.Value = row["StudentNumber"];
						sqlParameter3.Direction = ParameterDirection.Input;
						sqlParameter3 = sqlCommand5.Parameters.Add("@SemesterId", SqlDbType.VarChar, 12);
						sqlParameter3.Value = _Term;
						sqlParameter3.Direction = ParameterDirection.Input;
