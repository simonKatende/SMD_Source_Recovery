...
Cooldown is enforced by a set-based query (`GetGuardiansInGeneralCooldown`): a guardian is in
cooldown if a `tbl_SmsReminderLog` row exists with `GuardianKey = @gk AND ReminderType =
'General' AND SentAt >= @cutoff` (`@cutoff = today − GeneralReminderCooldownDays`). There is no
separate pure cooldown helper — the SQL filter is the single source of truth, so we don't ship
a tested-but-unused function.
