...
- [ ] **Step 1: Create the `notes/migrations/` folder and migration file**
...
- [ ] **Step 2: Verify syntax (no build/run; SQL is read by SSMS at deploy time)**
...
- [ ] **Step 1: Add `DialogResult.OK` on successful send and `DialogResult.Cancel` on cancel**
...
- [ ] **Step 3: Stage and smoke-test for non-regression**
...
- [ ] **Step 1: Create `FeesContactLog` POCO**
...
- [ ] **Step 2: Create `WorklistRow` POCO**
...
- [ ] **Step 3: Create `FeesFollowUpService` skeleton with connection plumbing**
...
- [ ] **Step 4: Add `GetWorklist` method to the service**
...
- [ ] **Step 5: Add insert + history methods to the service**
...
- [ ] **Step 1: Create the empty UserControl with a `SplitContainerControl`**
...
- [ ] **Step 2: Create the matching `.resx` (DevExpress designer-style files need one even when empty)**
...
- [ ] **Step 1: Add filter bar and grid control fields**
...
- [ ] **Step 2: Wire the filter bar + grid inside `InitializeComponent`**
...
- [ ] **Step 3: Add Load/Refresh logic and row-coloring**
...
- [ ] **Step 2: Build the right pane in `InitializeComponent`**
...
- [ ] **Step 3: Hook up selection-changed to populate the right pane**
...
- [ ] **Step 1: Add fields and form panel**
...
- [ ] **Step 2: Build the form in `InitializeComponent`** (insert after the history grid setup, before the right-pane controls are added)
...
- [ ] **Step 3: Add save handler (non-SMS path only for now)**
...
- [ ] **Step 2: Build them in `InitializeComponent`**
...
- [ ] **Step 3: Toggle visibility based on outcome**
...
- [ ] **Step 4: Include promise fields in save**
...
- [ ] **Step 1: Branch the SMS path in `BtnSave_Click`**
...
- [ ] **Step 2: Add the handler — delegate to Save then advance**
...
- [ ] **Step 2: Create the matching empty `.resx`** (copy the template from Task 4 step 2)

- [ ] **Step 3: Add Settings button on the filter strip**
...
- [ ] **Step 1: Add a ribbon-page field**
...
- [ ] **Step 2: Wire the page in `InitializeComponent`**
...
- [ ] **Step 3: Add the usrFeesFollowUp control to MainForm and switch to it when the tab is selected**
...
- [ ] **Step 1: Ensure both DB tables exist on the test DB**
...
- [ ] **Step 2: Run through the design-doc smoke-test script** (`docs/superpowers/specs/2026-05-27-fees-crm-design.md` section 6):
...
- [ ] **Step 3: If any scenario fails, file fixes as additional commits on `feat/fees-crm`. If all pass, the branch is ready to merge.**
